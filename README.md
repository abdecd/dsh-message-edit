...
- **自由 CRUD + Fork**：在 Timeline 右列对已落定消息随意增、删、改，草稿实时显示「新增 · 编辑 · 删除」计数并可一键重置；点击 **Fork** 按草稿内容重建消息历史并生成新版本。以用户消息结尾时，该消息作为新提示排入新会话并生成新的助手回复；以助手消息结尾或历史为空时只创建分支、不生成回复。Fork 顶部可选择目标工作区和 DSH agent preset（如标准模式）；不选择时分别沿用源会话工作区与 preset。
- **级联策略**：
...
- `GET /message-edit?sessionId=<id>`：读取可编辑消息、可重试回合、完整版本树及可用的 DSH agent preset roster。
- `POST /message-edit`：执行 `edit`、`reroll`、`retry` 或 `fork`，返回已发布的新 Session ID。`fork` 的 `rows` 为有序的消息行（`kind` 为 `user` / `assistant.reasoning` / `assistant.response`，`text` 为文本），按行重建整个历史；以 `user` 行结尾时触发新回复。可选 `workspaceId` 将新版本的工作目录与会话挂接到指定工作区；可选 `agentPreset` 使用指定 DSH preset；省略时沿用源会话对应设置。
